...
#:
#: 校准值 **0.25**（2026-08 ⑤下杆机位感知改造，4 段样本扫描，见
#: ``_probe_out/probe_h_downswing_dtl_sweep.py``）：因下杆期 ``h`` 单调递减，
#: **阈值越低触发越晚**——0.50 时 DTL 样本 ⑤ 距 ④ 仅 5~7 帧（接近顶点，用户
#: 反馈偏早）；调低后：
#:   - 0bb16a97：⑤ 190→191（距④ 8 帧，距⑥ 5 帧，真实穿越）；
#:   - 470057ac：⑤ 99→100（距④ 8 帧，距⑥ 3 帧，真实穿越）；
#:   - c6f67f38：⑤=174 不变（其下杆期 ``h`` 未在击球前穿越任何阈值，本就兜底
#:     比例，对本阈值不敏感）；
#:   - 22030124：face-on，⑤=113 恒定（DTL 阈值免疫，用户此前验收值保持）。
#: 稳定平台 [0.22, 0.30] 结果相同；取中点 0.25。⚠️ 低于 0.20 会触发 ⑤/⑥ 间距
#: 守卫回退兜底比例（470057ac ⑤ 退化为 98 估算帧，反而更早），**不要**调更低。
H_DOWNSWING_DTL: Final[float] = 0.25
