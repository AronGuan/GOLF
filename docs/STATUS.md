...
| 项 | 值 |
|---|---|
| 单元测试 | **399 passed / 0 failed**（8 模块；基线 220 → 355 → 349 → 371 → 382 → 386 → D 方案 399） |
| 真实视频端到端 | 7/9 段跑通（2 段残缺 NO_SWING 确定性失败）；正面1 触发 2 条风险、DTL 段 swing_plane=51.6° |
| 机位自动判定 | 9/9 命中 ground-truth |
| 击球帧校正 | **12 段真实视频：9 段有效校正 + 新样本 120→115 命中真实击球**、其余 9 段与 v2 零回归、opens=1 |
...
| **M8：DTL 击球帧手动微调** | CLUBLITE refine 在 DTL 视角下 M1 运动峰偏差方向不固定（实测 ±5 帧、不固定方向），单一常量救不了；**DTL 视频击球帧须 ◀/▶ 手动微调**（指标保持原值） | 不投入算法改动；如未来用纯几何判据（如杆身端点）根治再开启 |
